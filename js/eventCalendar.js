...
function getMonthEvents(obj) {
  if (
    obj.startDate.year === currentYearNum &&
    obj.startDate.month === currentMonthNum
  ) {
    return obj;
  } else {
    return false;
...
function resetDaysContent() {
  if (allStorage.length > 0) {
    ...
    monthEvents.forEach(function (monthEvent) {
      var previousEvents = document.getElementsByClassName(
        "eventId" + monthEvent.id
      );
...
function showExpiredEvents() {
  ...
  let allLocalStorageEvents = JSON.parse(
    localStorage.getItem("localEventInfo")
  );
